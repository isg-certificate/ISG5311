...
| Edit files on Mantis in `nano`    |
| Connect Visual Studio Code to Mantis to edit files    |
| Connect your desktop to Mantis's file system to access files |
...
When executing programs it can be really useful to know *which* copy of a program is being executed, as sometimes there will be multiple versions installed on a given system (this is definitely true of Mantis). You can use the command `which` for this:
...
You'll note that each of these is found in a path found in your `PATH` variable. This will be less trivial and more important to be able to check when it comes to software used for data analysis on Mantis.  
...
`condition` describes the logic of the loop, e.g. how many times it should iterate, or a rule under which it should stop. In BASH we typically use `for` and `while` loops. A `for` loop iterates over a list of fixed length executing the same code each time. A `while` loop executes code repeatedly until some condition is no longer met. 
...
In this last case the syntax `$(command)` is a command substitution. It means run the command and insert the output in place (in this case, inside the `()` used to define the array). Also note that the array elements will be parsed based on whitespace, so if file names contain whitespace (you should *avoid* filenames wiht spaces always) this approach will break. 
...
A straightforward way to use Sublime to write scripts is to write code locally, and then paste it into documents using `nano`, or use `scp`/`rsync` to upload the documents to Mantis. These solutions are not exactly ideal, but they work in a pinch. It is possible to connect Sublime through an ssh tunnel to give it access to documents on Mantis, but we haven't yet shown you the tools to do that, and we'll introduce another, easier approach in a moment. 

#### Integrated development environments. 

IDEs are dedicated code editors with *lots* more features. [Visual Studio Code](https://code.visualstudio.com) is another program we recommend you download and install locally. It also has syntax highlighting, but you'll notice you can open up projects and see the entire directory structure, and if you want you can even open a terminal inside it. There is a plugin that will let you connect VS Code to Mantis and open and edit files remotely (discussed below). 

### Conveniently accessing files. 

Ok, so aside from using `nano` (or learning to be a `vim` power user), how can you conveniently access and edit files on Mantis? There are two relatively straightforward ways:

1. Visual Studio Code has an extension. This is **highly** recommended. You can connect VS Code to Mantis directly via SSH. You can open windows focusing on specific directories, visualize the directory structure, and create and edit files directly on Mantis. You can also open a Mantis terminal window so that you can test and execute code, all in VS Code. The [instructions for setting up VSCode on Mantis](https://cbc-uconn.github.io/hpc-docs/software.html#vscode) are a little ahead of this point in the course, but probably manageable if you want to try it. To connect it to Mantis also requires a [GitHub](https://github.com/) account. It will definitely make a bit more sense after the next chapter covering SLURM. 

2. You can use the [Open OnDemand desktop](https://ondemand.hpc.cam.uchc.edu). Log in with your CAM credentials and have a look. The desktop can be opened with `Interactive Apps -> Mantis Desktop`. As of 2026, this is pretty new, so we're not sure how folks will make use of it. Note that you can also open a terminal window there. 

::: {.callout-warning}
You should only use dedicated software for editing code (or CLI editors on Mantis). Editing using word processors (such as MS Word) will often lead to difficult to diagnose problems. Word processors often insert hidden characters and/or use line break characters incompatible with Linux. These will cause confusing errors when executing code. The first place to start if you think this might be the issue is to try `cat -A myscript.sh`. If `myscript.sh` contains any weird hidden characters, this will print them. Compare to a script you know works. Non-standard line breaks are often the culprit and this will reveal them. 
:::
...
Create this script with the title `commonWords.sh` on Mantis and change the permission so that you can execute it. 
...
3. `./commonWords.sh` This will execute the script, and Linux looks for the shebang to see which interpreter should be used. It also creates an isolated execution context. For options 1 and 2, the script does not need to have execute permission. To run it this way, execute permission is required. 
